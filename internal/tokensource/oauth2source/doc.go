...
// resolved via OIDC discovery (the ".well-known/openid-configuration" document) against a
// configured issuer URL, following an OAuth2/OIDC provider setup such as Keycloak. The resolved
// token endpoint is cached for the lifetime of the source, and discovery is retried on the next
// call whenever a previous attempt failed.
//
