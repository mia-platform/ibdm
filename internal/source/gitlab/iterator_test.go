...
func TestGroupsIterator(t *testing.T) {
	testCases := map[string]struct {
		handler       http.HandlerFunc
		expectedCount int
		expectErr     bool
		...
		"single page": {
			handler: func(w http.ResponseWriter, r *http.Request) {
				assert.Contains(t, r.URL.Path, "/api/v4/groups")
				w.Header().Set("x-total-pages", "1")
				jsonResponse(t, w, []map[string]any{{"id": float64(1)}})
			},
			expectedCount: 1,
...
func TestGroupResourcesIterator(t *testing.T) {
	testCases := map[string]struct {
		handler       http.HandlerFunc
		resource      string
		groupID       string
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			client := newTestGitLabClient(t, srv)
			it := client.newGroupResourcesIterator(tc.resource, tc.groupID)
	...
	t.Run("pages arrive incrementally", func(t *testing.T) {
		...
		client := newTestGitLabClient(t, srv)
		it := client.newGroupResourcesIterator(accessTokenResource, "5")
